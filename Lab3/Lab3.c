...
unsigned long MaxJitter = 0;             // largest time jitter between interrupts in usec
#endif
...
// inputs:  none
// outputs: none
unsigned long DAS_debug[50];
int debug_index = 0;
int weird_flag = 0;
unsigned long time_array[50];
unsigned long thisTime_array[50];
unsigned long lastTime_array[50];
...
void DAS(void){ 
  ...
  if(NumSamples < RUNLENGTH){   // finite time run
    ...
    if(FilterWork>1){    // ignore timing of first interrupt
      thisTime_array[debug_index] = thisTime;
      lastTime_array[debug_index] = LastTime;
      unsigned long diff = OS_TimeDifference(LastTime,thisTime);
      // unsigned long diff = thisTime - LastTime;
      if (diff > 45000 || diff < 35000){
        weird_flag += 1;
        DAS_debug[debug_index] = diff;
      }
      debug_index += 1;
      if (debug_index == 50){
        debug_index = 0;
...
unsigned long input;  
  if(NumSamples < RUNLENGTH){   // finite time run
    PE0 ^= 0x01;
    input = 1;//ADC_In();           // channel set when calling ADC_Init <---uncomment
    PE0 ^= 0x01;
    DASoutput = Filter(input);
    FilterWork++;        // calculation finished
    PE0 ^= 0x01;
...
unsigned long thisTimeSW1 = 0;
unsigned long thisTimeSW2 = 0;
unsigned long lastTimeSW1 = 0;
unsigned long lastTimeSW2 = 0;
//************SW1Push*************
...
void SW1Push(void){
	...
	thisTimeSW1 = OS_MsTime();
  if(OS_TimeDifference(lastTimeSW1,thisTimeSW1) > 200){ // debounce
    if(OS_AddThread(&ButtonWork,100,1)){
      NumCreated++; 
    }
		lastTimeSW1 = thisTimeSW1;  // at least 20ms between touches
  }
}
//************SW2Push*************
...
// background threads execute once and return
void SW2Push(void){
	thisTimeSW2 = OS_MsTime();
  if(OS_TimeDifference(lastTimeSW2,thisTimeSW2) > 200){ // debounce
    if(OS_AddThread(&ButtonWork,100,1)){
      NumCreated++; 
    }
		lastTimeSW2 = thisTimeSW2;
  }
}
//--------------end of Task 2-----------------------------
...
void Consumer(void){ 
  ...
  ADC_Collect(5, 80000000/FS, &Producer); // start ADC sampling, channel 5, PD2, 400 Hz  <---uncomment
  NumCreated += OS_AddThread(&Display,128,0); 

  while(NumSamples < RUNLENGTH) { 
  	GPIO_PORTB_DATA_R ^= 0x8;//PB3
      //PE2 = 0x04;
...
// outputs: none
void Display(void){ 
  //OS_bWait(&display_sema);
	GPIO_PORTB_DATA_R ^= 0x80;//PB7
unsigned long data,voltage;
  ...
  //ST7735_Message(0,3,"ADC=",ADC_In()); 
  //ST7735_Message(0,3,"ADC=",input); 
    //PE3 = 0x00;
  } 
  //OS_bSignal(&display_sema);
  OS_Kill();  // done
} 
...
//1. button
//2. systick
//3. periodic interrupts
//4. foreground threads (pendSV)
...
//********initialize communication channels
  ...
  OS_InitSemaphore(&display_sema,1);
  OS_InitSemaphore(&button_sema,1);
...
#endif
  //ADC_Open(4);  // sequencer 3, channel 4, PD3, sampling in DAS() <--come back and deal with this later (Michael)
  //ADC_Open(0);  // sequencer 3, channel 4, PD3, sampling in DAS() <--come back and deal with this later (Michael) <--uncomment
  OS_AddPeriodicThread(&DAS,40000,1); // 2 kHz real time sampling of PD3
  //OS_EnableInterrupts();
  // OS_AddPeriodicThread(&DAS,PERIOD,0); // 2 kHz real time sampling of PD3
...
// create initial foreground threads
  ...
  OS_Launch(TIME_10MS); // doesn't return, interrupts enabled in her2
...
void Thread1(void){
  Count1 = 0;          
  for(;;){
    GPIO_PORTD_DATA_R ^= 0x01;       // heartbeat
    Count1++;
...
void Thread2(void){
  Count2 = 0;          
  for(;;){
    GPIO_PORTD_DATA_R ^= 0x02;       // heartbeat
    Count2++;
    OS_Suspend();      // cooperative multitasking
  }
}
void Thread3(void){
  Count3 = 0;          
  for(;;){
    GPIO_PORTD_DATA_R ^= 0x04;       // heartbeat
   // PE3 ^= 0x08;
    Count3++;
    OS_Suspend();      // cooperative multitasking
  }
...
void Thread6(void){  // foreground thread
 ...
 extern void Jitter(void);   // prints jitter information (write this)    <--- come back to this jitter function (Michael)
