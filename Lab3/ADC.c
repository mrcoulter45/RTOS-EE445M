...
// The code below has been copied and modified from the ADCT0ATrigger_4C123 ValvanoWare project
#include <stdint.h>
...
void ADC_Open(uint8_t channelNum){
  ...
  SYSCTL_RCGCADC_R |= 0x01;     // activate ADC0 --> works for PE2, PE3
  ...
  ADC0_EMUX_R &= ~0xF000; // timer trigger event
...
int ADC_Collect(uint32_t channelNum, uint32_t period, void(*intask)(unsigned long)){
  start_collection = 1;
	task = intask;
    volatile uint32_t delay;
  // **** GPIO pin initialization ****
  ...
  ADC0_IM_R |= 0x04;             // enable 2 interrupts
  ADC0_ACTSS_R |= 0x04;          // enable sample sequencer 2
  NVIC_PRI4_R = (NVIC_PRI4_R&0xFFFFFF1f)|0x00000040; //priority 2
...
//   TIMER0_TAMR_R = 0x00000002;   // configure for periodic mode, default down-count settings
//   TIMER0_TAPR_R = 0;            // prescale value for trigger
//   TIMER0_TAILR_R = 8000000-1;    // start value for trigger <-- manually set a default period
//   TIMER0_IMR_R = 0x00000000;    // disable all interrupts
//   TIMER0_CTL_R |= 0x00000001;   // enable timer0A 32-b, periodic, no interrupts
//   ADC0_PC_R = 0x01;         // configure for 125K samples/sec
//   ADC0_SSPRI_R = 0x3210;    // sequencer 0 is highest, sequencer 3 is lowest
