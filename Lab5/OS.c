...
int num_PCBs = 0;//number of PCBs active
PCB pcbs[NUMPROCESSES];
int PCB_index = -1;

void PCB_TCB_Init(void){
...
void SetInitialStack2(int i){
	...
	if(PCB_index > -1){
		Stacks[i][STACKSIZE-11] = (int32_t)pcbs[PCB_index].data_ptr;       //R9 --> location of start of ELF program
	}
	else{
...
int OS_AddThread(void(*task)(void), unsigned long stackSize, unsigned long priority){
	...
	//if non-process thread, will send 0x090909 for data. 
	//If process thread, will send location of start of ELF program for data
	if(PCB_index == -1){
		SetInitialStack2(thread_IDs[thread_IDs_index]);
	    //bits [31:16] = process priority
	    //bits [15:0] = thread priority
	    tcbs[thread_IDs[thread_IDs_index]].Priority = (1 << 16) + priority;
	    tcbs[thread_IDs[thread_IDs_index]].ProcessID = -1;
	}
	else{//thread from user program that must have parent process
		SetInitialStack2(thread_IDs[thread_IDs_index]);
		pcbs[PCB_index].num_threads += 1;
	    //bits [31:16] = process priority
	    //bits [15:0] = thread priority
	    tcbs[thread_IDs[thread_IDs_index]].Priority = (pcbs[PCB_index].Priority << 16) + priority;
	    tcbs[thread_IDs[thread_IDs_index]].ProcessID = PCB_index;
	}
...
// entry - starting address of user program's first thread
// text  - starting address of user code on the heap
// data  - starting address of user code's data (variables) on the heap
int OS_AddProcess(void(*entry)(void), void *text, void *data, unsigned long stackSize, int priority){
	int i = 0;
	...
	if(i == NUMPROCESSES){
		return 0; //max processes activated, no more processes allowed
	}
	num_PCBs += 1;
	pcbs[PCB_index].data_ptr = data;
	pcbs[PCB_index].code_ptr = text;
	pcbs[PCB_index].ID = PCB_index;
	pcbs[PCB_index].Priority = priority;
	// pcbs[PCB_index].num_threads += 1;
	pcbs[PCB_index].active = 1;
...
void OS_Kill(void)
{
	if(RunPt->ProcessID > -1){
		pcbs[RunPt->ProcessID].num_threads -= 1;
		if(pcbs[RunPt->ProcessID].num_threads == 0){
			//kill process
			pcbs[RunPt->ProcessID].active = 0;
			num_PCBs -= 1;
			//heap free user function data and code
			OS_bWait(&Heap_Sema);
			if(Heap_Free(pcbs[RunPt->ProcessID].data_ptr)){//Heap_Free returns 0 if no error
				UART_OutString("heap error\r\n");
			}
			if(Heap_Free(pcbs[RunPt->ProcessID].code_ptr)){
				UART_OutString("heap error\r\n");
			}
			OS_bSignal(&Heap_Sema);
		}
