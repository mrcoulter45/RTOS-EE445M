...
// // inputs:  none
// // outputs: none
// // unsigned long LastTime;  // time at previous ADC sample
...
//       //PE0 ^= 0x01;
//       //input = ADC_In();           // channel set when calling ADC_Init <---uncomment
//       input = 1;           // channel set when calling ADC_Init 
//       //PE0 ^= 0x01;
...
long jitter = 0;                    // time between measured and expected, in us
  if(NumSamples < RUNLENGTH){   // finite time run
    ...
    input = 1;
		unsigned long reload = TIMER2_TAILR_R;   // 4) reload value
    // PE0 ^= 0x01;
    thisTime = OS_Time();       // current time, 12.5 ns
    DASoutput = Filter(input);
    FilterWork++;        // calculation finished
...
int Testmain5(void){       // Testmain5 Lab 3
  ...
  //NumCreated += OS_AddThread(&Thread7,128,1);   <- fix later (Michael)
