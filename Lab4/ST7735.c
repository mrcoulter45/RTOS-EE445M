...
extern Sema4Type SSI_sema;
Sema4Type screen_lock;
...
void ST7735_InitR(enum initRFlags option) {
  ...
  OS_InitSemaphore(&screen_lock,1);
}
...
//needed to reappropriate fputc for Lab4
...
//ST7735_DivideScreen
//Split the screen into 2 logical displays
void ST7735_DivideScreen ()
{
  //screen divider - same height as one line of text
	OS_bWait(&SSI_sema);
  ST7735_DrawFastHLine(0, 80, 128, ST7735_WHITE);
  ST7735_DrawFastHLine(0, 81, 128, ST7735_WHITE);
  ST7735_DrawFastHLine(0, 82, 128, ST7735_WHITE);
  ST7735_DrawFastHLine(0, 83, 128, ST7735_WHITE);
  ST7735_DrawFastHLine(0, 84, 128, ST7735_WHITE);
  ST7735_DrawFastHLine(0, 85, 128, ST7735_WHITE);
  ST7735_DrawFastHLine(0, 86, 128, ST7735_WHITE);
	OS_bSignal(&SSI_sema);
}
//ST7735_Message
//int device = which of the 2 logical screens to use - valid inputs: 1,2
//int line = which line on the device to use - valid inputs: 0,1,2,3
//char* string = the message to display, aka name of the thing being displayed by string
//int32_t value = a value to display
void ST7735_Message (int device, int line, char *string, int32_t value)
{
  OS_bWait(&screen_lock);
  OS_bWait(&SSI_sema);
  int screen_line = line;
  screen_line += device*9;
	ST7735_SetCursor(0,screen_line);
  if(string != NULL){
    ST7735_DrawFastHLine(0, (screen_line * 10) + 0, 128, ST7735_BLACK);//blacks out whatever the previous line was
    ST7735_DrawFastHLine(0, (screen_line * 10) + 1, 128, ST7735_BLACK);
    ST7735_DrawFastHLine(0, (screen_line * 10) + 2, 128, ST7735_BLACK);
    ST7735_DrawFastHLine(0, (screen_line * 10) + 3, 128, ST7735_BLACK);
    ST7735_DrawFastHLine(0, (screen_line * 10) + 4, 128, ST7735_BLACK);
    ST7735_DrawFastHLine(0, (screen_line * 10) + 5, 128, ST7735_BLACK);
    ST7735_DrawFastHLine(0, (screen_line * 10) + 6, 128, ST7735_BLACK);
    ST7735_DrawFastHLine(0, (screen_line * 10) + 7, 128, ST7735_BLACK);
