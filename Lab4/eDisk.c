...
DSTATUS eDisk_Init(BYTE drv){
  ...
  ty = 0;
	BYTE send_cmd_result = send_cmd(CMD0, 0);
  if ( send_cmd_result == 1) {      /* Put the card SPI/Idle state */
...
DRESULT eDisk_Write(BYTE drv, const BYTE *buff, DWORD sector, UINT count){
  ...
  if (count == 1) {  /* Single sector write */
		int cmd_response = send_cmd(CMD24,sector);
		int xmit_response = xmit_datablock(buff, 0xFE); // PROBLEM HERE
    if (cmd_response == 0  /* WRITE_BLOCK */
      && xmit_response)
      count = 0;
  }
